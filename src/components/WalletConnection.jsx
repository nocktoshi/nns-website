...
const WalletConnection = ({ provider, onAccountChange }) => {
  ...
  const [walletType] = useState(() => detectWalletType());
  ...
  // Show appropriate install message based on what's available
  if (!walletType) {
    return (
      ...
      {account ? (
        ...
        <>
          ...
          >
            ...
            {isConnecting
              ? "Connecting..."
              : `${walletType === 'iris' ? 'Iris' : 'Rose'} Connect`
            }
          </Button>
        </>
      )}
